...
8. **الأرقام دائماً غربية (0-9) لا عربية-هندية** — لا تكتب رقماً عربياً-هندياً في مصدر، ولا تُنسّق تاريخاً بـ`'ar'` أو `'ar-SA'`؛ استخدم `'ar-u-nu-latn'`. `tests/numerals.test.ts` يحرس الاثنين
...
| Vitest | 226 اختبار وحدة (`tests/`) — تُشغَّل قبل كل دفع |
