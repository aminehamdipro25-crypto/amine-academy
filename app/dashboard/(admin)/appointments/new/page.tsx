...
export default function NewAppointmentPage() {
  ...
  useEffect(() => {
    fetch('/api/admin/clients-list')
      ...
      .catch(() => setError('تعذر تحميل المشتركين'))
  ...
  async function submit() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setSuccess(`تم إنشاء الموعد! رابط الجلسة: ${d.appointment?.meetingUrl}`)
  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      <div>
        <h1 className="text-2xl font-black text-gray-900">جدولة موعد جديد</h1>
        <p className="text-gray-500 text-sm mt-1">سيُرسل تأكيد تلقائي للمشترك وسيُنشأ رابط Jitsi Meet</p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-6 space-y-5">
        ...
        {/* Type */}
        <div>
          <label className="block text-xs font-bold text-gray-500 mb-2">نوع الجلسة</label>
          <div className="grid grid-cols-3 gap-3">
            {[
              { val: 'followup',   label: 'متابعة',        color: 'border-brand-400 bg-brand-50 text-brand-700' },
              { val: 'assessment', label: 'تقييم أولي',     color: 'border-purple-400 bg-purple-50 text-purple-700' },
              { val: 'emergency',  label: 'استشارة عاجلة', color: 'border-red-400 bg-red-50 text-red-700' },
            ].map(({ val, label, color }) => (
        ...
        <div className="flex items-start gap-3 bg-green-50 border border-green-200 rounded-xl p-4">
          ...
          <div className="text-sm text-green-700">
            <span className="font-bold">رابط Jitsi Meet سيُنشأ تلقائياً</span> — مجاني، لا يحتاج حساباً، يعمل في المتصفح مباشرة. سيُرسل في بريد التأكيد.
          </div>
        </div>
        ...
        <div>
          ...
          <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={3}
            placeholder="أي معلومات إضافية للمشترك..."
