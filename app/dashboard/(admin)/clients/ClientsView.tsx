...
export default function ClientsView({ parents, error }: {
  ...
  const STATUS: Record<string, { label: string; dot: string; badge: string }> = {
    active:    { label: t.status.active,    dot: 'bg-green-500',  badge: 'bg-green-100 text-green-700 ring-1 ring-green-200' },
    pending:   { label: t.status.pending,   dot: 'bg-amber-400',  badge: 'bg-amber-100 text-amber-700 ring-1 ring-amber-200' },
    suspended: { label: t.status.suspended, dot: 'bg-red-500',    badge: 'bg-red-100 text-red-700 ring-1 ring-red-200' },
    cancelled: { label: t.status.cancelled, dot: 'bg-gray-400',   badge: 'bg-gray-100 text-gray-600 ring-1 ring-gray-200' },
    expired:   { label: t.status.expired,   dot: 'bg-orange-400', badge: 'bg-orange-100 text-orange-700 ring-1 ring-orange-200' },
  }

  const PLAN_LABELS: Record<string, string> = {
    basic: t.plan.basic, standard: t.plan.standard, premium: t.plan.premium,
                ...
                <div className="flex items-center justify-between pt-3 border-t border-gray-50">
                  <div className="flex items-center gap-3">
                    <div className="text-center">
                      <p className="text-[10px] text-gray-400 font-medium">{t.planFieldLabel}</p>
                      <p className="text-xs font-black text-gray-700 mt-0.5">{plan || '—'}</p>
                    </div>
                    <div className="w-px h-6 bg-gray-100" />
                    <div className="text-center">
                      <p className="text-[10px] text-gray-400 font-medium">{t.childrenFieldLabel}</p>
                      <p className="text-xs font-black text-gray-700 mt-0.5 ltr-num">{childCount}</p>
                    </div>
                    <div className="w-px h-6 bg-gray-100" />
                    <div className="text-center">
                      <p className="text-[10px] text-gray-400 font-medium">{t.registrationFieldLabel}</p>
