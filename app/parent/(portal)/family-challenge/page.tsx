...
export default function FamilyChallengePage() {
  const { lang } = useLang()
  const t = tr[lang].parentFamilyChallenge
  const [childName, setChildName] = useState(t.defaultChildName)
    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-black text-gray-900 flex items-center gap-2">
          <Users className="w-5 h-5" style={{ color: '#7C5CFC' }} /> {t.pageTitle}
        </h1>
      ...
      {stage === 'intro' && (
        <div className="rounded-2xl p-6 text-center" style={{ background: '#FFFFFF', border: '1px solid #F0E8FF' }}>
          ...
          <h2 className="font-black text-gray-900 text-base mb-2">{t.introTitle(childName)}</h2>
          ...
          <div className="flex flex-col h-full">
            <div className="px-5 pt-5 pb-2 text-center flex-shrink-0">
              <span className="inline-block bg-white/10 text-white font-black text-xs px-4 py-1.5 rounded-full">
                {stage === 'parent-turn' ? t.parentTurnBadge : t.childTurnBadge(childName)}
              </span>
            </div>
      ...
      {stage === 'result' && parentResult && childResult && (
        <div className="rounded-2xl p-6 text-center" style={{ background: '#FFFFFF', border: '1px solid #F0E8FF' }}>
          ...
          <h2 className="font-black text-gray-900 text-base mb-1">
            {winner === 'tie' ? t.tieResultTitle : winner === 'parent' ? t.parentWinTitle : t.childWinTitle(childName)}
          </h2>
          <p className="text-gray-400 text-xs mb-5">{t.resultFooter}</p>
          ...
          <div className="grid grid-cols-2 gap-4 mb-5">
            <div className="rounded-xl p-4" style={{ background: winner === 'parent' ? '#F0FDF4' : '#F9FAFB', border: winner === 'parent' ? '1.5px solid #A7F3D0' : '1px solid #F3F4F6' }}>
              ...
              <div className="text-xs text-gray-500 mt-1">{t.parentLabel}</div>
            </div>
          ...
          >
            <RotateCcw className="w-4 h-4" /> {t.playAgainButton}
          </button>
        </div>
      )}
